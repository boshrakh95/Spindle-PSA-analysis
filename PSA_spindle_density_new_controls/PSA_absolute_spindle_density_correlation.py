# For computing abs power, we have to sum the absolute power columns for all the sub-bands in that frequency band
...
sigma_band_columns = ['total_NREM_act_abs_ROI_C3C4_11_12', 'total_NREM_act_abs_ROI_C3C4_12_13',
                      'total_NREM_act_abs_ROI_C3C4_13_14', 'total_NREM_act_abs_ROI_C3C4_14_15',
                      'total_NREM_act_abs_ROI_C3C4_15_16']
band_name = 'Sigma'
...
def process_and_plot():
    ...
    if df_sumo is not None and df_a7 is not None and df_martin is not None and df_psa is not None:
        ...
        # Sum the absolute power columns for the sigma band
        df_psa['sigma_abs_power'] = df_psa[sigma_band_columns].sum(axis=1)
        ...
        df_psa = df_psa[['filename', 'sigma_abs_power']]
        ...
        df_merged.dropna(subset=['SUMO_value', 'A7_value', 'Martin_value', 'sigma_abs_power'], inplace=True)
        ...
        plt.subplot(1, 3, 1)
        if show_regression:
            X = df_merged['sigma_abs_power']
            y = df_merged['SUMO_value']
            X = sm.add_constant(X)
            model = sm.OLS(y, X).fit()
            r_squared = model.rsquared
            p_value = model.pvalues[1]
            sns.scatterplot(x='sigma_abs_power', y='SUMO_value', data=df_merged, color='#4D71A3', alpha=0.7)
            sns.regplot(x='sigma_abs_power', y='SUMO_value', data=df_merged, color='#2E7D32',
                        label=f'All data: R²={r_squared:.2f}, p={p_value:.2e}')
            plt.legend()
        else:
            sns.scatterplot(x='sigma_abs_power', y='SUMO_value', data=df_merged, color='#4D71A3', alpha=0.7)
        plt.title(f'{band_name} Absolute Power vs SUMO N2 Density')
        plt.xlabel(f'{band_name} Absolute Power')
        ...
        if show_regression:
            ...
            r_squared = model.rsquared
            p_value = model.pvalues[1]
            sns.scatterplot(x='sigma_abs_power', y='A7_value', data=df_merged, color='#4D71A3', alpha=0.7)
            sns.regplot(x='sigma_abs_power', y='A7_value', data=df_merged, color='#2E7D32',
                        label=f'All data: R²={r_squared:.2f}, p={p_value:.2e}')
            plt.legend()
        else:
            sns.scatterplot(x='sigma_abs_power', y='A7_value', data=df_merged, color='#4D71A3', alpha=0.7)
        plt.title(f'{band_name} Absolute Power vs A7 N2 Density')
        plt.xlabel(f'{band_name} Absolute Power')
        ...
        if show_regression:
            ...
            r_squared = model.rsquared
            p_value = model.pvalues[1]
            sns.scatterplot(x='sigma_abs_power', y='Martin_value', data=df_merged, color='#4D71A3', alpha=0.7)
            sns.regplot(x='sigma_abs_power', y='Martin_value', data=df_merged, color='#2E7D32',
                        label=f'All data: R²={r_squared:.2f}, p={p_value:.2e}')
            plt.legend()
        else:
            sns.scatterplot(x='sigma_abs_power', y='Martin_value', data=df_merged, color='#4D71A3', alpha=0.7)
        plt.title(f'{band_name} Absolute Power vs Martin N2 Density')
        plt.xlabel(f'{band_name} Absolute Power')
        plt.ylabel('Martin N2 Density')
        plt.grid(True)

        plt.tight_layout()
        plot_filename = os.path.join(destination_folder, f'correlation_{band_name.lower()}_abs_power_n2_density.pdf')
