...
def process_and_plot():
    ...
    if df_sumo is not None and df_a7 is not None and df_martin is not None:
        ...
        # Extract the relevant columns from SUMO (keep control_group for potential future use)
        ...
        single_color = '#2ECC71'  # it doesn't work, whatever I choose for regression line applies to all points
        ...
        if show_regression:
            ...
            # Plot all points with a single color (green) and no label
            sns.scatterplot(x='SUMO_value', y='A7_value', data=df_merged, color=single_color, alpha=0.7)
            # Add regression line in black
            sns.regplot(x='SUMO_value', y='A7_value', data=df_merged, color='green',
        ...
        if show_regression:
            ...
            p_value = model.pvalues[1]
            # Plot all points with a single color (green) and no label
            sns.scatterplot(x='SUMO_value', y='Martin_value', data=df_merged, color=single_color, alpha=0.7)
            # Add regression line in black
            sns.regplot(x='SUMO_value', y='Martin_value', data=df_merged, color='green',
                        label=f'All data: R²={r_squared:.2f}, p={p_value:.2e}')
            plt.legend()
        else:
            sns.scatterplot(x='SUMO_value', y='Martin_value', data=df_merged, color=single_color, alpha=0.7)
        ...
        if show_regression:
            ...
            p_value = model.pvalues[1]
            # Plot all points with a single color (green) and no label
            sns.scatterplot(x='A7_value', y='Martin_value', data=df_merged, color=single_color, alpha=0.7)
            # Add regression line in black
            sns.regplot(x='A7_value', y='Martin_value', data=df_merged, color='green',
                        label=f'All data: R²={r_squared:.2f}, p={p_value:.2e}')
            plt.legend()
        else:
            sns.scatterplot(x='A7_value', y='Martin_value', data=df_merged, color=single_color, alpha=0.7)
        ...
        plot_filename = os.path.join(destination_folder, 'correlation_SUMO_A7_Martin_all_data.pdf')
...
###################################
# the following code tries to differentiate between the original and new control groups by color,
# but it doesn't work as expected and everything is green (modify later if you need the same plot with different colors)
...
#             sns.scatterplot(x='SUMO_value', y='A7_value', hue='control_group', data=df_merged, palette=palette,
#                             alpha=0.7)
#
#             # Add regression line in a darker color (green)
#             sns.regplot(x='SUMO_value', y='A7_value', data=df_merged, color='#2E7D32',  # Darker green
#                         label=f'All data: R²={r_squared:.2f}, p={p_value:.2e}')
#             plt.legend()
#         else:
#             sns.scatterplot(x='SUMO_value', y='A7_value', hue='control_group', data=df_merged, palette=palette,
...
#
#             # Plot points with different colors for each control group
#             sns.scatterplot(x='SUMO_value', y='Martin_value', hue='control_group', data=df_merged, palette=palette,
#                             alpha=0.7)
#
#             # Add regression line in dark green
...
#
#             # Plot points with different colors for each control group
#             sns.scatterplot(x='A7_value', y='Martin_value', hue='control_group', data=df_merged, palette=palette,
#                             alpha=0.7)
#
#             # Add regression line in dark green
...
#
#         # Save the plot as a PDF file
#         plot_filename = os.path.join(destination_folder, 'correlation_SUMO_A7_Martin_with_control_groups_with_stats.pdf')
#         plt.savefig(plot_filename, format='pdf', bbox_inches='tight')
#         plt.close()
#         print(f"\nPlot saved as {plot_filename}")
#     else:
