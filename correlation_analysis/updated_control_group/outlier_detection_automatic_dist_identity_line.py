...
destination_folder = os.path.join(data_folder, 'automatic_outliers_identity_line')
distance_threshold = 1.5  # Threshold for outlier detection based on distance from the identity line
...
def calculate_distance_from_identity_line(x, y):
    """Calculate the perpendicular distance from points to the identity line x = y."""
    return np.abs(x - y) / np.sqrt(2)

def detect_outliers(df_pair, detector1, detector2):
    """Detect outliers based on distance from the identity line x = y."""
    distances = calculate_distance_from_identity_line(df_pair[f'{detector1}_value'], df_pair[f'{detector2}_value'])
    ...
    reasons = [f"Distance from identity line: {distance:.2f} > {distance_threshold}" for distance in outliers['distance']]
    ...
    if df_sumo is not None and df_a7 is not None and df_martin is not None:
        ...
        for detector1, detector2 in detector_pairs:
            ...
            if len(outliers) > 0:
                ...
                max_val = max(df_pair[f'{detector1}_value'].max(), df_pair[f'{detector2}_value'].max())
                min_val = min(df_pair[f'{detector1}_value'].min(), df_pair[f'{detector2}_value'].min())
                plt.xlim(min_val, max_val)
                plt.ylim(min_val, max_val)

                # Plot the identity line x = y
                plt.plot([min_val, max_val], [min_val, max_val], color='green', linestyle='--', label=f'Identity Line x = y')
